...
const MenuCompQuartiers:React.FC<MenuCompQuartiersProps>=(props:MenuCompQuartiersProps)=>{
    ...
    const gestRecalculStationnementAgreg=async()=>{
        ...
        try {
            const success= await serviceAnalyseInventaire.recalculeInventaireBackend()
            if (!success)throw new Error('Échec de recalcul de l\'inventaire')
        } catch (e) {
            console.error(e)
    ...
    const gestRecalculAncil=async()=>{
        props.defCalculEnCours(true)
        try {
            const success=await serviceAnalyseInventaire.recalculeDonneesFoncieresBackend()
            if (!success)throw new Error('Échec de recalcul de l\'inventaire')
        } catch (e) {
            console.error(e)
